...
public class PemilikModel extends ModelAbstract{
    ...
    public void insertData(PemilikEntity pemilik) {
        try {
            sql = "INSERT INTO pemilik (nama_pemilik, password, noTelp, alamat) Values(?,?,?,?)";
            PreparedStatement stat = conn.prepareStatement(sql);
            stat.setString(1, pemilik.getNama());
            stat.setString(2, pemilik.getPassword());
            stat.setString(3, pemilik.getNoTelp());
            stat.setString(4, pemilik.getAlamat());
    ...
    public void updateNama(String nama, int id) {
        try {
            sql = "update pemilik SET nama_pemilik =? WHERE id_pemilik =?";
            PreparedStatement stat = conn.prepareStatement(sql);
    ...
    public int cekLogin(String nama, String password) {
        int cek = 0;
        try {
            sql = "SELECT * FROM pemilik WHERE nama_pemilik =? and password=?";
            PreparedStatement stat = conn.prepareStatement(sql);
            stat.setString(1,nama);
